...
def anchored_fine_video_sparse_attn(
    ...
    *,
    ...
    if sequence != parent_blocks * PARENT_WIDTH:
        raise ValueError("Anchored Fine-VSA requires the native 64-token tiled Wan path")
    ...
    exact_output, _ = fine_sparse_attention(
        ...
        selected_counts=(selection.selected_actual_kv_tokens // CHILD_WIDTH),
    )
    ...
    decision = AnchoredFineVSADecision(
        ...
        fine_tail_anchor_overlap_tokens=(selection.fine_tail_anchor_overlap_tokens),
    )
    return output.transpose(1, 2), decision


def summarize_anchored_decision(decision: AnchoredFineVSADecision, ) -> dict[str, Any]:
    anchor_descriptors = (decision.anchor_parent_blocks * CHILDREN_PER_PARENT)
    ...
    nominal_sparsity = 1.0 - (SELECTED_CHILD_BLOCKS * CHILD_WIDTH / (decision.parent_blocks * PARENT_WIDTH))
    return {
        "parent_blocks":
        ...
        decision.anchor_parent_blocks,
        "anchor_child_descriptors":
        anchor_descriptors,
        "fine_tail_child_descriptors":
        fine_descriptors,
        "selected_child_blocks":
        SELECTED_CHILD_BLOCKS,
        "nominal_anchor_kv_tokens": (decision.anchor_parent_blocks * PARENT_WIDTH),
        "nominal_fine_kv_tokens":
        fine_descriptors * CHILD_WIDTH,
        "nominal_kv_tokens":
        NOMINAL_KV_TOKENS,
        ...
        nominal_sparsity,
        "anchor_valid_tokens_min":
        int(decision.anchor_actual_kv_tokens.min().item()),
        "anchor_valid_tokens_max":
        int(decision.anchor_actual_kv_tokens.max().item()),
        "fine_valid_tokens_min":
        int(decision.fine_actual_kv_tokens.min().item()),
        "fine_valid_tokens_max":
        int(decision.fine_actual_kv_tokens.max().item()),
        "total_valid_tokens_min":
        int(decision.selected_actual_kv_tokens.min().item()),
        "total_valid_tokens_max":
        int(decision.selected_actual_kv_tokens.max().item()),
        "native_valid_tokens_min":
        int(decision.native_actual_kv_tokens.min().item()),
        "native_valid_tokens_max":
        int(decision.native_actual_kv_tokens.max().item()),
        "actual_kv_token_error_abs_max":
        int(token_error.max().item()),
        "fine_tail_anchor_overlap_tokens_max":
        int(decision.fine_tail_anchor_overlap_tokens.max().item()),
        "budget_definition": ("fixed 1000 eight-token descriptors; top native parent anchors "
