...
def fine_video_sparse_attn(
    ...
    if sequence != parent_blocks * PARENT_WIDTH:
        raise ValueError(
            "Fine-VSA requires the native 64-token tiled Wan path"
        )
    ...
    selected_actual_kv_tokens = child_sizes[
...
def summarize_fine_vsa_decision(
    decision: FineVSADecision,
) -> dict[str, Any]:
    token_error = (
    ...
    return {
        ...
        "child_width": decision.child_width,
        "selected_child_blocks": decision.selected_child_blocks,
        "nominal_kv_tokens": (
            decision.selected_child_blocks * decision.child_width
        ),
        ...
        "active_child_count_min": int(
            decision.active_child_count.min().item()
        ),
        "active_child_count_max": int(
            decision.active_child_count.max().item()
        ),
        "native_actual_kv_tokens_min": int(
            decision.native_actual_kv_tokens.min().item()
        ),
        "native_actual_kv_tokens_max": int(
            decision.native_actual_kv_tokens.max().item()
        ),
        "selected_actual_kv_tokens_min": int(
            decision.selected_actual_kv_tokens.min().item()
        ),
        "selected_actual_kv_tokens_max": int(
            decision.selected_actual_kv_tokens.max().item()
        ),
        "actual_kv_token_error_abs_max": int(token_error.max().item()),
        "budget_definition": (
            "1000 fixed 8-token descriptors; valid-token support matched "
